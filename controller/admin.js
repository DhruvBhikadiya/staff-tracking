...
const leaveModel = require("../model/leave.js");
...
module.exports.getAllManager = async (req, res) => {
  try {
    ...
    if (checkAdmin) {
      ...
      const managerData = await userModel
        .find({ role: 'Manager' })
        .skip(skip)
        .limit(limit);
      ...
      if (managerData.length > 0) {
        return res.status(200).json({
          msg: managerData,
          currentPage: page,
          totalPages: Math.ceil(totalManager / limit),
          totalManager,
          status: 0,
          response: "success",
        });
...
module.exports.getAllUser = async (req, res) => {
  try {
    ...
    if (key === "all") {
      ...
      filter = { managerId: key };
    } else {
      return res
        .status(400)
        .json({ msg: "Missing or invalid key parameter", status: 1, response: "error" });
    }
    ...
    if (userData.length > 0) {
      ...
      return res
        .status(404)
        .json({ msg: "Users not found", status: 1, response: "error" });
    }
  } catch (e) {
    console.error(e);
    return res
...
module.exports.dashboard = async (req, res) => {
  try {
    ...
    if (checkAdmin) {
      ...
      const totalNonAdminUsers = await userModel.countDocuments({
        role: { $in: ['Manager', 'User'] }
      });

      const totalOrder = await orderModel.countDocuments();

      const totalPayment = await userModel.aggregate([
        {
          $group: {
            _id: null,
      ...
      const totalAllowance = await allownceModel.aggregate([
        {
          $group: {
            _id: null,
            totalAllowance: { $sum: "$amount" }
          }
        }
      ]);

      if (totalNonAdminUsers || totalOrder || totalPayment || totalAllowance) {
        return res.status(200).json({
          totalNonAdminUsers,
          totalOrder,
          totalPayment: totalPayment[0].totalPayment,
          totalAllowance: totalAllowance[0].totalAllowance,
          status: 0,
          response: "success",
        });
      } else {
...
module.exports.getUserLeave = async (req, res) => {
  try {
    ...
    const userLeaveData = await leaveModel.find({ managerId: req.user.id });
...
module.exports.updateUserLeave = async (req, res) => {
  try {
    ...
    if (!updatedLeave) {
      return res
        .status(404)
        .json({ msg: "Leave not found", status: 1, response: "error" });
    }
