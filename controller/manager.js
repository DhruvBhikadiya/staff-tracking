  ...
  try {
    ...
    if (checkManager) {
      ...
      const userData = await userModel
        .find({ role: 'User', managerId: req.user.id })
        .skip(skip)
        .limit(limit);

      const totalUsers = await userModel.countDocuments({ role: 'User', managerId: req.user.id });
...
module.exports.chnage_password = async (req, res) => {
  try {
    if (req.body.npass == req.body.cpass) {
      checkemail = await userModel.findOne({ email: req.user.email });
      if (checkemail) {
        const password = await bcrypt.hash(req.body.cpass, 10);
        let changepass = await userModel.findByIdAndUpdate(checkemail.id, {
          password: password,
...
module.exports.addLeave = async (req, res) => {
  try {
    ...
    const userLeave = await leaveModel.create(newLeave);
    ...
    console.error("Error in addLeave:", e);
    return res
      .status(500)
      .json({ msg: "Something went wrong", status: 1, response: "error" });
  }
...
module.exports.getLeave = async (req, res) => {
  try {
    ...
    const userLeaveData = await leaveModel.find({ userId: req.user.id });
